...
PRE_CACHE_PATH=/data/oppo_log/pre_cache
...
function initOplusLog(){
    ...
    if [ "${PANICE_NABLE}" = "true" ] || [ x"${CAMERA_PANIC_ENABLE}" = x"true" ]; then
        ...
        isPreLogCache=`getprop persist.sys.cache.log`
        if [ x"${isPreLogCache}" == x"true" ]; then
            setprop ctl.stop preLogCache
        fi
...
function transferDataOppoLog(){
    ...
    if [ -d  ${DATA_OPLUS_LOG} ]; then
        ALL_SUB_DIR=`ls ${DATA_OPLUS_LOG} | grep -v DCS | grep -v data_vendor | grep -v TMP`
        for SUB_DIR in ${ALL_SUB_DIR};do
...
function transferDataTMP(){
    DATA_TMP_LOG=/data/oppo/log/TMP
...
function getSystemStatus() {
    ...
    if [ x${boot_completed} == x"1" ]; then
        ...
        dumpsys package --da > ${outputPath}/dumpsys_package_da.txt
...
function preLogCache(){
    panicenable=`getprop persist.sys.assert.panic`
    camerapanic=`getprop persist.sys.assert.panic.camera`
    if [ "${panicenable}" != "true" ] && [ x"${camerapanic}" != x"true" ] ; then
        logdsize=`getprop persist.logd.size`
        if [ "${logdsize}" = "" ]; then
            /system/bin/logcat -G 5M
        fi
        mkdir -p ${PRE_CACHE_PATH}
        mkdir -p ${PRE_CACHE_PATH}/apps
        rm -rf ${PRE_CACHE_PATH}/apps/*
        chmod -R 777 ${PRE_CACHE_PATH}
        preAndroidSize=20480
        preAndroidCount=10
        /system/bin/logcat -f ${PRE_CACHE_PATH}/apps/android.txt -r${preAndroidSize} -n ${preAndroidCount}  -v threadtime -A
    else
        setprop ctl.stop preLogCache
    fi
}

function transferPreversionLogCache(){
    isPreLogCache=`getprop persist.sys.cache.log`
    if [ x"${isPreLogCache}" == x"true" ]; then
        stoptime=`getprop sys.oppo.log.stoptime`
        preLogCachePath="${SDCARD_LOG_BASE_PATH}/log@stop@${stoptime}"
        mkdir -p ${preLogCachePath}/recovery_log
        mv ${PRE_CACHE_PATH}/apps/* ${preLogCachePath}/recovery_log
    fi
}
case "$config" in
    "transfer_log")
